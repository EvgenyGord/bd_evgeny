...
class MovieModelTestCase(TestCase):
    ...
    def test_movie_delete_record(self):
        # Проверка удаления записи из БД
        self.print_info('Start test_movie_delete_record')
        movies = Movie.objects.all()
        self.assertEqual(len(movies), 3)
        self.print_info('Finish test_movie_delete_record')

    def test_movie_change_record(self):
        # Проверка изменения записи из БД
        self.print_info('Start test_movie_change_record')
        movies = Movie.objects.all()
        movies.filter(name="Mask").update(year=2022)
        self.assertEqual(Movie.objects.get(name="Mask").year, 2022)
        self.print_info('Finish test_movie_change_record')
...
class StudentModelTestCase(TestCase):
    ...
    def setUp(self):
        print('-' * 20)
        self.print_info('Start setUp')
        self.student = Student.objects.create(first_name='Test Student')
        Student.objects.create(first_name='GG')
        Student.objects.create(first_name='Mask')
        self.print_info('Finish setUp')
